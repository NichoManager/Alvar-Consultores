...
const publicStatusLabels: Record<
  string,
  string
> = {
  available: 'Disponible',
...
export function normalizePropertyFilterText(
  value: string,
) {
  return value
    .normalize('NFD')
...
export function buildPropertyLocationOptions(
  properties: Property[],
) {
  const options =
    new Map<
  ...
  properties.forEach(
    (property) => {
      ...
      if (area) {
        const id =
          `area:${provinceKey}:${cityKey}:${areaKey}`;
...
export function propertyFiltersFromSearchParams(
...
export function getActivePropertyFilters(
  filters: PropertyFilterState,
) {
  const chips: Array<{
    key:
