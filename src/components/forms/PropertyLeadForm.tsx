...
export function PropertyLeadForm({ propertyTitle }: { propertyTitle: string }) {
  ...
  return (
    <form className="premium-form property-lead-form" onSubmit={submit} noValidate>
      ...
      <FormField label="Nombre" name="leadName" autoComplete="name" required error={errors.leadName} />
      <FormField label="Teléfono" name="leadPhone" type="tel" autoComplete="tel" required error={errors.leadPhone} />
      <FormField label="Email" name="leadEmail" type="email" autoComplete="email" error={errors.leadEmail} />
      <PrivacyField name="propertyPrivacy" error={errors.propertyPrivacy} />
      <Button type="submit">Solicitar información</Button>
      <p className="form-note">Canal online en preparación. Teléfono y WhatsApp están disponibles para una respuesta directa.</p>
    </form>
  );
}
