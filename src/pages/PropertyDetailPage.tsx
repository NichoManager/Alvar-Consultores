...
export function PropertyDetailPage() {
  ...
  return (
    <>
      ...
      <InternalHero
        eyebrow={`SELECCIÓN ORIENTATIVA · ${property.area} · ${property.city}`}
        ...
        aside={(
          <div className="property-hero__summary">
            ...
            <strong>{property.price ? `${property.price.toLocaleString('es-ES')} €` : 'Consultar precio'}</strong>
          </div>
        )}
      />

      <section className="property-detail-gallery" aria-label={`Galería de ${property.title}`}>
        <Container>
        ...
        <Container className="property-content__grid">
          <article>
            ...
            >
              ...
              <span className="map-placeholder__pin">{property.city.charAt(0)}</span>
          ...
          <aside>
            ...
            <a className="aside-phone" href={`tel:${business.phoneMobileHref}`}>Llamar al {business.phoneMobile}</a>
            <a
      ...
      <div className="mobile-property-cta">
        <a href={`tel:${business.phoneMobileHref}`}>Llamar</a>
        <a href={whatsappUrl(business.phoneMobileHref, message)} target="_blank" rel="noreferrer">
