-- Allow admins to define the editorial order of featured properties.
-- Existing featured properties remain visible and keep their previous relative
-- order because featured_position is nullable.

alter table public.properties
  add column if not exists featured_position integer null;

alter table public.properties
  add constraint properties_featured_position_positive
  check (
    featured_position is null
    or featured_position >= 1
  );

comment on column public.properties.featured_position is
  'Optional manual position in the Home featured-properties section. Position 1 is the principal card.';

-- PostgreSQL cannot replace a function when its table return type changes.
-- Recreate the public RPC preserving its filters, privacy rules and grants.
...
comment on function public.get_public_properties(text, boolean, integer) is
  'Read-only public property API. Returns published/reserved rows, featured ordering metadata, optional SEO metadata and media while redacting private addresses.';
